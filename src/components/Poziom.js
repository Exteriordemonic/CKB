...
const Data = [
  {
    title: "Adept",
    content:
      "Grupa Adept to Twój pierwszy krok w treningu z odważnikami kulowymi, w okresie od 4 do 6 miesięcy poznasz 6 podstawowych technik hardstyle kettlebell według metodyki Strong First, tj. dead lift(martwy ciąg), swing, clean(zarzut), tgu(tureckie wstawanie), goblet squat(przysiad), military press(wyciskanie żołnierskie), snatch(rwanie). Podczas początkowych zajęć możesz spodziewać się dużej ilości ćwiczeń wzmacniających z masą własną ciała, korygujących błędne nawyki i wzorce ruchowe. Nauczysz się prawidłowo oddychać i wytwarzać napięcie mięśniowe, tak byś mógł/mogła bezpiecznie zacząć ćwiczenia z odważnikiem. Priorytetem jest nauka techniki i przygotowanie Cię do bardziej wymagających treningów z dwoma odważnikami.",
  },
  {
    title: "Active",
    content:
      "Grupa skierowana do osób aktywnych, które przeszły przez pierwszy etap szkolenia i mogą już wykonywać ćwiczenia na dwa odwazniki. Jednocześnie celem tej grupy nie jest osiąganie wyników sportowych, a wykorzystanie kettli do utrzymania dobrej formy, zdrowa i pozytywnej energii w życiu codziennym.",
  },
  {
    title: "Advanced",
    content:
      "Grupa zaawansowana realizuje cele sportowe z nastawieniem na osiąganie wyników siłowych i starty w zawodach hardstyle kettlebell. Do tej grupy mogą dołączyć studenci po wstępnym okresie szkolenia w grupach Adept i Active.",
  },
  {
    title: "Personal",
    content:
      ...
      <ContainerStyled>
          ...
          <TransitionGroup className="image-wrapper">
            ...
            >
              <LVL>
                ...
                <PoziomText className="text">{Data[card].content}</PoziomText>
                <LinkStyled
                  className="subtitle bold"
                  as="a"
                  href="tel:+48508378847"
                >
        ...
        <Wrapper mobile>
          ...
          {Data.map((value, index) => {
            return (
              <LVL key={index}>
                <PoziomTitle className="title bold">
                  {Data[index].title}
                </PoziomTitle>
                <PoziomText className="text">{Data[index].content}</PoziomText>
              </LVL>
            )
          })}
